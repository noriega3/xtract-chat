...
function M.encode(data, key, alg)
    ...
    if type(key) ~= 'string' and type(key) ~= 'number' then return nil, "Argument #2 must be string" end
...
function M.decode(data, key, verify)
    ...
    if verify and type(key) ~= 'string' and type(key) ~= 'number' then return nil, "Argument #2 must be string" end
