...
let client, emitter, actions, socket, roomPath, roomName

before(function(){
	return new Promise((resolve, reject) => {
		args.score = Math.random(0,100000000)
		args.avatar = Math.random(0,100)
		roomPath = 'source:blackjack:classic'
...
it('reserve a game room', function(done){

	actions.findAndReserveRoom(roomPath, {isGameRoom: true, isTurnBased:true})

	emitter.once('reservation', function(res){
		expect(res).be.an('object')
		res.should.have.nested.property('roomName')
		res.roomName.should.have.string(roomPath)

		res.should.have.nested.property('message')
		res.message.should.have.string('Reserved the seat for game room: '+roomPath)

		res.should.have.nested.property('params')
		res.params.should.include({isGameRoom: true, isTurnBased:true})
...
it('subscribed', function(done) {
	actions.requestSubscribe(roomName, {isGameRoom: true, isTurnBased:true})
	...
	emitter.on('subscribed', function(res){
		...
		if(_isEqual(roomName, res.room)){
			clearTimeout(timer)
			res.response.should.include({sessionId: client.getSessionId(), isGameRoom: true, isTurnBased:true})
			done()
		}
	})
})
