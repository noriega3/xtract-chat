...
before(function(){
	return new Promise((resolve, reject) => {
		...
		roomPath = 'source:slots:enchantedForest'
...
it('reserve a game room', function(done){
	...
	actions.findAndReserveRoom(roomPath, {isGameRoom: true})
	...
	emitter.once('reservation', function(res){
		...
		res.params.should.include({isGameRoom: true})
...
it('subscribed', function(done) {
	actions.requestSubscribe(roomName, {isGameRoom: true})
	...
	emitter.on('subscribed', function(res){
		...
		if(_isEqual(roomName, res.room)){
			clearTimeout(timer)
			res.response.should.include({sessionId: client.getSessionId(), isGameRoom: true})
			done()
		}
	})
})
