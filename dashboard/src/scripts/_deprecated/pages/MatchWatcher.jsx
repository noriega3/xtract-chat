...
class Server extends Component {
	...
	handleWsReconnect(e){
		...
		if(!this.state.connected){
			this.ws = new WebSocket(process.env.WS_CONNECTOR_URL, null, {
				protocolVersion: 13,
				origin: process.env.WS_ORIGIN_URL,
			})
			this.ws.onopen = e => {
...
*/}
											...
											<Panel>
												...
												<div className={"mui--text-subhead"}>
													...
													<div>
														...
														{(_.get(srv,'pm2_env.status') === "online" && <Button color={'danger'} value={_.get(srv,'name')} disabled={!connected || _.get(srv,'name') === 'websocket-server'}  onClick={this.handleServerStop}>Stop</Button>)}
													</div>
												</div>
											</Panel>
										</Col>)
					...
					<Tab value="gcm" label="Push Notifications">
						{(connected && tabIndex === 3) ? (
							<Panel>
								<div className="mui--text-subhead">Select an id</div>
							</Panel>
						) : (<Panel><span>Not Connected</span></Panel>)}
					</Tab>
				</Tabs>
