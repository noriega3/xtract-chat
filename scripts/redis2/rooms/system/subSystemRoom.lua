...
local roomTypeId            = _tonumber(redis.call('hget', rk.roomInfo, 'roomTypeId'))
local isBot                 = redis.call('hget', rk.session, 'bot')

--check if room type matches
if(roomTypeId ~= -1) then
	return redis.error_reply('INVALID ROOM TYPE')
end
--========================================================================
-- UTILITY Functions
--========================================================================
...
local subscribeToRoom = function()
	...
	if(roomExists) then
		...
		if(not memberIndex or #memberIndex < 1) then return false, 'FULL' end
		local seat = memberIndex[1]:gsub("open:seat:", "")
		redis.call('zrem', rk.roomName, "open:seat:"..seat)

		--add hex of taken:seatIndex:sessionId:###:## / taken:sessionId:seatIndex:###:##
	...
	return false, 'NO EXIST - '..clientRoomName
end
...
for x=1, #funct do
	status, err = funct[x]()
	if(not status) then	return redis.error_reply(err or status)  end
end
