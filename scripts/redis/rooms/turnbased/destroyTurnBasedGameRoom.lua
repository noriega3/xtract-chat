...
local delList = {rk.roomMatchState, rk.roomTurnState, rk.roomName, rk.roomInfo, rk.roomHistory, rk.roomMessages, rk.roomBots, rk.roomReserves, rk.roomOptIns}

--remove optIns and players from current game Id and next game id
local optInList = redis.call('lrange', rk.roomOptIns, 0, -1)

if(optInList and #optInList > 0) then
	for x=1, #optInList do
		delList[#delList+1] = optInList[x]

		if(redis.call('exists', optInList[x]..":locked") == 1) then
			delList[#delList+1] = optInList[x]..":locked"
		end
