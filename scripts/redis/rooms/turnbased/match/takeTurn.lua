...
local turnMessage		= ARGV and ARGV[1] or {}
...
local rk = {
	...
	roomPlayersMatch  	= "rooms|"..KEYS[1].."|players:",
...
local gameInfo = redis.call('hmget', rk.roomInfo, 'gameId', 'turnSeatIndex', 'turnExpireAt')
local gameId = gameInfo[1]
local gameState = redis.call('lindex', rk.roomMatchState, 0)
local turnSeatIndex = tonumber(gameInfo[2])
local turnExpireAt = tonumber(gameInfo[3])
local nextSeat
...
--set key
rk.roomPlayersMatch = rk.roomPlayersMatch..gameId

local numSeats = redis.call('hlen', rk.roomPlayersMatch)

local function isSeatActive(seatIndex)
	if(seatIndex == 5) then return true end
	local isSeatTaken = redis.call('hget', rk.roomPlayersMatch, seatIndex)
