...
const BotBlackJack = require('../../scripts/bots/bot-blackjack')
const BotSlots = require('../../scripts/bots/bot-slots')
	...
	return Promise.filter(revRoomList, ([type, room]) => {
		...
		if (intendedGameRoom) {
			return client.filterMultipleGameRooms(sessionId)
				.then((rooms) => {
					const unsubscribe	= require('../../scripts/jobs/unsubscribe')
...
shared.setupRoom = (sessionId = 'server', roomName, roomType = getRoomTypeFromParams(), roomParams = {test2: '1'}) => {
	...
	_log('room arr is ', helper._roomNameToArr(roomName))
	let roomProps = {
		roomType,
		...
		...helper._roomNameToArr(roomName)
	}
...
shared.findAndDestroyRoom = (roomName) => {
	...
	const roomNameKey = helper._bar('rooms', roomName)
	const shouldDestroy = () => {
...
shared.checkForGameRoomsAndUnSub = ([sessionId, roomList]) => {
	const unsubscribe = require('../jobs/unsubscribe')
